...
| 페이지 | URL | 용도 | 권장 기기 | 필요 권한 |
|--------|-----|------|-----------|-----------|
| **고객 디스플레이** | `/stream-customer` | 고객에게 주문 내역과 결제 금액 표시 | 태블릿 (고객 방향) | 로그인 필요 |
| **카운터 (주문)** | `/order` | 직원이 고객 주문을 입력 | PC 또는 태블릿 | 주문 권한 |
| **주방 디스플레이** | `/cooking` | 주방에서 주문 확인 및 완료 처리 | 모니터 또는 태블릿 | 주방 권한 |
| **관리자** | `/admin-check` | 매출 현황 및 통계 확인 | PC | 관리자 |
| **직원 관리** | `/admin-maker` | 직원 계정 생성 및 권한 설정 | PC | 관리자 |
| **메뉴 관리** | `/admin-food` | 메뉴 및 카테고리 등록/수정 | PC | 관리자 |
...
```
- 새 주문이 들어오면 자동으로 "대기중" 목록에 추가
- 주문번호, 메뉴, 수량, 옵션 한눈에 확인
- "조리 시작" 버튼 → "조리중" 상태로 변경
- "조리 완료" 또는 "바로 완료" 버튼으로 완료 처리
- 완료 내역 버튼으로 이전 주문 확인 가능
